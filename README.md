...
- **Requirement & Usage:**
  ...
  - ❗(Regarding Server Connection): Note that, since this is the user client for our sketching system, you also need to connect to **our listening server** that runs the neural network (TGP-Net) to handle the prediction request before using our software. **<ins>Thus, you MUST use this client with internet connection.</ins>** However, since the TGP-Net Listening Server is currently only configured on our personal computer which might be sometimes shut down, the connection may not be established (if so, you may see some warnings in the software). <ins>**In this case, please feel free to contact me to start the server for you when you need to use our sketching software.**</ins>
